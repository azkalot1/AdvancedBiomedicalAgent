...
class EnrichedSearchOutput(BaseModel):
    status: str
    results: list[EnrichedSearchResult] | None = None
    error: str | None = None
...
class UnifiedSearchInput(BaseModel):
    ...
    def check_valid_use_case(self) -> 'UnifiedSearchInput':
        is_property_lookup = (
...
async def _search_structured_sections_async(
    async_config: AsyncDatabaseConfig, set_id_ids: list[int], search_input: UnifiedSearchInput, model: SentenceTransformer
) -> list:
    if not set_id_ids:
        return []
...
async def _search_dailymed_structured_async(
    async_config: AsyncDatabaseConfig, set_ids: list[str], search_input: UnifiedSearchInput, model: SentenceTransformer
) -> list:
    if not set_ids:
        return []
...
async def _discover_drug_ids_by_keyword_async(
    async_config: AsyncDatabaseConfig, search_input: UnifiedSearchInput
) -> tuple[list[int], list[str]]:
...
async def _group_and_enrich_discovery_results(
    ...
    for product_name, sections in grouped_by_drug.items():
        ...
        enriched_results.append(
            EnrichedSearchResult(product_name=product_name, properties=properties, sections=deduplicated_sections)
        )
    return enriched_results


async def _handle_property_lookup(async_config: AsyncDatabaseConfig, drug_names: list[str]) -> list[EnrichedSearchResult]:
    tasks = [_find_drug_properties_async(async_config, name) for name in drug_names]
    all_properties = await asyncio.gather(*tasks)
    return [EnrichedSearchResult(product_name=p.name, properties=p) for p in all_properties if p and p.name]


async def _handle_label_search(
    async_config: AsyncDatabaseConfig, search_input: UnifiedSearchInput, model: SentenceTransformer
) -> list[EnrichedSearchResult]:
    if search_input.drug_names:
        ...
        for drug_name in search_input.drug_names:
            ...
            aggregated_result = EnrichedSearchResult(
...
async def unified_search_async(db_config: DatabaseConfig, search_input: UnifiedSearchInput) -> EnrichedSearchOutput:
    try:
        ...
        return EnrichedSearchOutput(status=status, results=results or None, error=error)
    except Exception as e:
        print(f"Caught unexpected exception: {e}")
        return EnrichedSearchOutput(status="error", error=f"An unexpected server error occurred: {type(e).__name__}")
